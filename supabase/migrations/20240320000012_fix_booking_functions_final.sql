...
-- Create base booking function with proper date handling
...
BEGIN
  ...
  v_check_in_date := p_check_in::date;
  v_check_out_date := p_check_out::date;
  ...
  SELECT EXISTS (
    ...
    AND check_out::date = v_check_in_date
  ) INTO v_is_extension;

  -- Apply booking rules if not an extension
  IF NOT v_is_extension AND v_check_in_date < CURRENT_DATE + 3 THEN
