...
-- First, drop EVERYTHING related to auth/profiles/whitelist
...
-- NUCLEAR RESET: Fix ALL existing users
DO $$
BEGIN
  -- First, ensure admin is in whitelist
  INSERT INTO whitelist (email, notes, has_seen_welcome)
  VALUES ('andre@thegarden.pt', 'Admin user', true)
  ON CONFLICT (email) DO UPDATE SET 
    has_seen_welcome = true;

  -- Reset ALL user metadata to default state
  UPDATE auth.users
