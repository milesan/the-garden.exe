...
-- Create base booking function with fixed date comparison
...
BEGIN
  ...
  -- Check if dates are available using explicit date range comparison
  IF EXISTS (
    SELECT 1 FROM availability 
    WHERE accommodation_id = p_accommodation_id 
    AND status IN ('BOOKED', 'HOLD')
    AND date::date >= v_check_in_date::date
    AND date::date < v_check_out_date::date
  ) THEN
  ...
  -- Mark dates as booked in availability using explicit date handling
  ...
  FROM generate_series(
    v_check_in_date::date,
    (v_check_out_date - INTERVAL '1 day')::date,
    INTERVAL '1 day'
  ) d
...
-- Create manual booking function with proper error handling
